...
{
	// You can use R(row, jcolumn) to get the value at the cross position in row-th row and column-th column of the relational matrix, and this function returns - 1 if it is over the boundary.
	if (row >= 0 && row < dimension && column >= 0 && column < dimension)
		return matrix[row * dimension + column];
	else
		return -1;
...
bool Relation::getEquivalenceClasses(int* result) const
{
